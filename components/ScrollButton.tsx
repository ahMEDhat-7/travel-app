...
export default function ScrollButton({ targetId }: { targetId: string }) {
  ...
  return (
    ...
    >
      <svg className="w-6 h-6 animate-bounce text-amber-200/70 group-hover:text-amber-200 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
      </svg>
      <span className="absolute -bottom-6 text-xs text-amber-200/50 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Explore</span>
    </button>
  );
}