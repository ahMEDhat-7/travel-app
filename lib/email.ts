...
<body style="font-family:'Helvetica Neue',Arial,sans-serif;background-color:#f5f5f5;margin:0;padding:20px">
  <div style="max-width:600px;margin:0 auto;background-color:#ffffff;border-radius:12px;overflow:hidden">
    <div style="background:${headerGradient};padding:30px;text-align:center">
      <h1 style="color:${headerColor};margin:0;font-size:28px">${headerTitle}</h1>
    </div>
    <div style="padding:30px">${bodyHtml}</div>
    <div style="background-color:#1a1a1a;padding:20px;text-align:center">
      <p style="color:#888888;margin:0;font-size:12px">&copy; 2026 Sharm Cloud Tours. All rights reserved.</p>
    </div>
  </div>
</body>
</html>`;
...
export async function sendPasswordReset(options: {
  ...
  const html = layoutHtml('Reset Your Password', `
    ...
    <p style="color:#999999;font-size:12px;margin-top:30px">If the button doesn&apos;t work, copy and paste this link into your browser:<br />${resetUrl}</p>
  `);
...
export async function sendBookingConfirmation(options: {
  ...
  const html = layoutHtml('Booking Confirmed!', `
    <p style="color:#333333;font-size:16px;margin-bottom:20px">Dear <strong>${customerName}</strong>,</p>
    ...
    <div style="background-color:#f9f9f9;border-radius:8px;padding:20px;margin:20px 0">
      ...
      <p style="color:#555555;margin:10px 0"><strong>Booking ID:</strong> ${bookingId}</p>
      <p style="color:#555555;margin:10px 0"><strong>Tour:</strong> ${tourName}</p>
      <p style="color:#555555;margin:10px 0"><strong>Date:</strong> ${tourDate}</p>
      <p style="color:#555555;margin:10px 0"><strong>Guests:</strong> ${people}</p>
      <p style="color:#555555;margin:10px 0"><strong>Total Price:</strong> ${currency} ${totalPrice.toFixed(2)}</p>
    </div>
  ...
  return sendMail({ to, subject: `Booking Confirmed - ${tourName}`, html });
}
...
export async function sendBookingStatusUpdate(options: {
  ...
  const notesHtml = adminNotes ? `
    <div style="background-color:#fffbeb;border-radius:8px;padding:20px;margin:20px 0;border-left:4px solid #FFD700">
      <p style="color:#333333;margin:0"><strong>Note from Admin:</strong><br />${adminNotes}</p>
    </div>
  ` : '';

  const html = layoutHtml(`Booking ${statusText}`, `
    <p style="color:#333333;font-size:16px;margin-bottom:20px">Dear <strong>${customerName}</strong>,</p>
    <p style="color:#333333;font-size:16px">${statusMessage}</p>
    <div style="background-color:#f9f9f9;border-radius:8px;padding:20px;margin:20px 0">
      <h3 style="color:#000000;margin-top:0">Booking Details</h3>
      <p style="color:#555555;margin:10px 0"><strong>Booking ID:</strong> ${bookingId}</p>
      <p style="color:#555555;margin:10px 0"><strong>Tour:</strong> ${tourName}</p>
      ...
      <p style="color:#555555;margin:10px 0"><strong>Total Price:</strong> ${currency} ${totalPrice.toLocaleString()}</p>
...
export async function sendAdminBookingNotification(options: {
  ...
  const notesHtml = notes ? `<p style="color:#555555;margin:10px 0"><strong>Notes:</strong> ${notes}</p>` : '';
  ...
  const html = layoutHtml('New Booking Received!', `
    ...
    <div style="background-color:#f9f9f9;border-radius:8px;padding:20px;margin:20px 0">
      ...
      <p style="color:#555555;margin:10px 0"><strong>Date:</strong> ${tourDate}</p>
      <p style="color:#555555;margin:10px 0"><strong>Guests:</strong> ${people}</p>
      <p style="color:#555555;margin:10px 0"><strong>Total Price:</strong> $${totalPrice.toFixed(2)}</p>
    </div>
    <div style="background-color:#f9f9f9;border-radius:8px;padding:20px;margin:20px 0">
      <h3 style="color:#000000;margin-top:0">Customer Details</h3>
      <p style="color:#555555;margin:10px 0"><strong>Name:</strong> ${contactName}</p>
      <p style="color:#555555;margin:10px 0"><strong>Email:</strong> ${contactEmail}</p>
      <p style="color:#555555;margin:10px 0"><strong>Phone:</strong> ${contactPhone}</p>
      ${notesHtml}
    </div>
  ...
  return sendMail({ to: adminEmail, subject: `New Booking: ${tourName} - ${people} guests`, html });
}
